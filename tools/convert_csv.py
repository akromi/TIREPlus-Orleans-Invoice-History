...
"""
import csv, datetime, html, json, re, sys, os
...
def convert(inv_csv, item_csv, out_dir, cust_csv=None, veh_csv=None):
    ...
    def write(name, var, cols, rows, source=None, cutoff=None):
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write("window.%s = {\n\"meta\": %s,\n\"cols\": %s,\n\"rows\": [\n"
                    % (var, json.dumps(meta), json.dumps(cols)))
            f.write(",\n".join(json.dumps(r, ensure_ascii=False, separators=(",", ":")) for r in rows))
            f.write("\n]};\n")
        print("wrote %s (%d rows, %.1f MB)" % (path, len(rows), os.path.getsize(path) / 1e6))

    inv_cutoff = max((r[4] for r in inv_rows if r[4]), default="")
    write("invoices.js", "TP_INVOICES", INV_COLS, inv_rows, inv_csv, inv_cutoff)
    # Newest date this items export really covers. A single stray covered job
    # can sit well past the export date, so take the last date at which a
    # rolling window of recent invoices is still mostly covered.
    item_jobs = {r[0] for r in item_rows}
    dated = sorted(((r[4], r[0] in item_jobs) for r in inv_rows if r[4]), key=lambda x: x[0])
    item_cutoff, window, covered = "", [], 0
    for day, ok in dated:
        window.append(ok)
        covered += ok
        if len(window) > 40:
            covered -= window.pop(0)
        if len(window) >= 20 and covered * 2 >= len(window):
            item_cutoff = day
    write("items.js", "TP_ITEMS", ITEM_COLS, item_rows, item_csv, item_cutoff)
    if cust_csv:
        write("customers.js", "TP_CUSTOMERS", CUST_COLS, cust_rows, cust_csv, "")
    if veh_csv:
        write("vehicles.js", "TP_VEHICLES", VEH_COLS, veh_rows, veh_csv, "")
